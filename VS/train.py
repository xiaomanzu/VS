...
# from contrastive_loss import PLLLoss
import sys
...
#RUN:
#Y_true tensor([0, 5, 4, 2, 4, 5, 3, 1, 2, 4, 3, 2, 6, 2, 1, 3], device='cuda:0')

torch.set_printoptions(precision=2, sci_mode=False)

parser = argparse.ArgumentParser(description='PyTorch implementation of ICLR 2022 Oral paper PiCO')
# parser.add_argument('--dataset', default='VLCS', type=str,
#                     choices=['PACS','VLCS','officehome'],
#                     help='dataset name (cifar10)')
parser.add_argument('--dataset', default='SEED', type=str,
                    choices=['PACS','VLCS','SEED'])
parser.add_argument('--target-domain',default='0',type=str)
parser.add_argument('--exp-dir', default='experiment/PiCO-SEED', type=str,
                    help='experiment directory for saving checkpoints and logs')
parser.add_argument('--data_aug',default='freq_mod_data',choices=['reuse_data', 'mult_data', 'noise_data', 'neg_data', 'freq_mod_data'])
...
parser.add_argument('-lr_decay_epochs', type=str, default='25,38',
                    help='where to decay lr, can be a list')
parser.add_argument('-lr_decay_rate', type=float, default=0.1,
                    help='decay rate for learning rate')
parser.add_argument('--cosine',  default=True,
                    help='use cosine lr schedule')#action='store_true',
...
parser.add_argument('--domain-class', default=15, type=int,
...
parser.add_argument('--prot_start', default=1, type=int,
                    help = 'Start Prototype Updating') # TODO: 1
...
# How to run : CUDA_VISIBLE_DEVICES=0 python -u train97.py --multiprocessing-distributed  --num-class 7
# CUDA_VISIBLE_DEVICES=0 python -u train97.py --multiprocessing-distributed  --num-class 5  --target-domain SUN --dataset VLCS --batch-size 64
def main():
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'PACS':
        ...
        train_loader, train_givenY, train_sampler, test_loader = load_PACS(target_domain,partial_rate=args.partial_rate, batch_size=args.batch_size )

    elif args.dataset == 'VLCS':
        target_domain= args.target_domain
        train_loader, train_givenY, train_sampler, test_loader = get_vacs_domain(target_domain,partial_rate=args.partial_rate, batch_size=args.batch_size)

    elif args.dataset == 'SEED':
        target_domain = args.target_domain
        if args.not_pbd:
            train_loader, train_data, test_loader = get_SEED(target_domain, batch_size=args.batch_size)
        else:
            train_loader, train_givenY, train_sampler, test_loader = get_SEED_domain(target_domain,partial_rate=args.partial_rate, batch_size=args.batch_size)
    else:
...
def train_vanilla(train_loader,model_vanilla,vanilla_fn, optimizer,  epoch):
    model_vanilla.train()
    loss = 0
    for i,(data_, label_, person )in enumerate(train_loader):
        # label=torch.LongTensor(label_)
        data,label =data_.cuda(),label_.cuda()
        optimizer.zero_grad()
        input, _,_= model_vanilla(data)
        vanilla_loss = vanilla_fn(input, label.long())
        vanilla_loss.backward()
        optimizer.step()
        loss += vanilla_loss.item()
        # print(',Epoch[%d/30 loss=%.4f' % (epoch + 1, loss.item()))
        acc = accuracy(input, label)[0]
        # acc.update(acc[0])
        if i % 100 == 0:
            print('epoch:%d   loss: %.3f   acc: %.3f' % (epoch + 1,  loss ,acc))
            loss=0
...
def train(train_loader, model ,loss_fn, loss_cont_fn,loss_domain_fn, optimizer,  epoch, args, tb_logger, start_upd_prot=False):
    ...
    loss_domain_log = AverageMeter('Loss@Domain', ':2.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, acc_cls, acc_proto, loss_cls_log,loss_domain_log, loss_cont_log],
        prefix="Epoch: [{}]".format(epoch))

    # switch to train mode
    model.train()
    end = time.time()
    for i, (images_w, images_s, labels, true_labels, index,domain_label) in enumerate(train_loader):
        # print('!!!!!!',images_w.shape, images_s.shape, labels.shape, true_labels.shape, index.shape,domain_label.shape)
        #VLCS torch.Size([16, 3, 224, 224]) torch.Size([16, 3, 224, 224]) torch.Size([16, 5]) torch.Size([16]) torch.Size([16]) torch.Size([16])
        #PACS torch.Size([32, 3, 224, 224]) torch.Size([32, 3, 224, 224]) torch.Size([32, 7]) torch.Size([32]) torch.Size([32]) torch.Size([32])
        data_time.update(time.time() - end)
        a,b=domain_label.min(),domain_label.max()
        ...
        # contrastive loss
        # 负样本，正样本
        # torch.Size([8320, 128]) torch.Size([64, 8320]) 64
        # 8320-64-64=8192

        loss_cont = loss_cont_fn(features=features_cont, mask=mask,domain_score_cont=domain_score_cont,
        domain_target_cont=domain_target_cont, batch_size=batch_size)
        # classification loss
    ...
    if args.gpu == 0:
        ...
        tb_logger.log_value('Domain Loss', loss_domain_log.avg, epoch)
